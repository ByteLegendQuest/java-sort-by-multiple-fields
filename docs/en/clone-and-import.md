...
`git clone https://github.com/ByteLegendQuest/java-sort-by-multiple-fields`

A project directory named `java-sort-by-multiple-fields` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-sort-by-multiple-fields` directory which is created just now.
